...
| **Artifact hygiene** | fixed | *"The spec is build scaffolding, not part of the product. The delivered code, docs, comments, tests, and any generated output must read as a standalone artifact — leave **no** `AC-id`s, build-phase / §-section numbers, spec ids or filenames, predecessor-component provenance, or 'newly / now / previously' build-increment framing in them. **Preserve every engineering rationale** — why a choice was made, pattern callouts, in-repo cross-refs that resolve in the shipped tree — and strip only the linkage to how the artifact was built."* — a future reader of the product must never be pointed back at the spec that produced it; `verify-spec`'s hygiene sweep flags any leak that slips through |
...
- **The artifact stands alone.** Every emitted driver carries the fixed *Artifact hygiene* rule, so the worker keeps the delivered code/docs/tests free of spec-linkage (`AC-id`s, phase/§ numbers, spec ids, predecessor provenance, build-increment framing) while preserving the engineering *why*. This is durable and cross-cutting — also promote it to `CLAUDE.md` so it's re-injected every turn. The backstop is `verify-spec`, which now runs a report-only spec-linkage hygiene sweep at the done-gate.
