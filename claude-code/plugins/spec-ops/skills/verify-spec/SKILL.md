...
- **On `CODEX: YES`, follow `${CLAUDE_PLUGIN_ROOT}/references/cross-model-judge.md`** for the full mechanics — final-pass-only, concurrent dispatch beside the Claude `spec-verify-judge` `Task`, the rubric-verbatim prompt, exit-code branching, AND-merge, and stubborn-split escalation. **verify-spec bindings for that policy:** `--kind judge-verify`; `--schema-file ${CLAUDE_PLUGIN_ROOT}/schemas/judge_verify.schema.json`; rubric `${CLAUDE_PLUGIN_ROOT}/agents/spec-verify-judge.md`; inputs = the target + the current ledger only (no memory of prior passes). The AND-merge folds the **union** of both judges' findings into the single existing `judge` field — `verdict: "complete"` only when **both** judges return `complete`, `missed` / `weakEvidence` = the union — so `stop_verify_spec.py` and the ledger schema stay unchanged.
