...
- `backwardSweep` and `specLinkageSweep` are the two **report-only sweeps**: like a `contradicted` claim they surface findings but **never block the stop**, and you edit nothing — the hook shape-validates the field but never gates on it; the judge attests the sweep ran. (This holds wherever either sweep appears below.)
...
Also write the **backward sweep** into the ledger's `backwardSweep`: set `ran` true, record the `base` swept (or `skippedReason`), and list every unmapped substantive hunk in `findings`. Likewise write the **spec-linkage hygiene sweep** into `specLinkageSweep` (set `ran` true, list the detector's findings) — both are report-only sweeps.

### 4. Judge — an independent agent confirms completeness

**The agent that did the verifying does not get to declare it complete.** Before you try to stop, dispatch a fresh **verification judge** — the **`spec-ops:spec-verify-judge`** agent (the `Task` tool, `subagent_type: spec-ops:spec-verify-judge`) — with **no memory of your passes**; hand it **only the target and the ledger**, not your reasoning. Its adversarial rubric lives in the agent: read-only, it re-derives the checkable claims from the target itself, flags any missing from the ledger (`missed`) — confirming every `AC-id` is present as a claim when the target has an Acceptance Criteria section — and flags any `confirmed`/`contradicted` claim whose evidence is hollow, stale, doc-based, or **below the standard the claim demands** (`weakEvidence`). It also attests the **backward** and **spec-linkage** sweeps ran honestly — against the right base, with the judgment pass for spec/history-named `identifier`s and inert `background` context, not just the greppable tokens — so a gap it spots there is its own pass, while those sweeps' *findings* never block. It returns strict JSON:

```json
{ "verdict": "complete | gaps", "missed": [...], "weakEvidence": [...], "backwardSweepAttested": true, "specLinkageSweepAttested": true }
```
