...
**This is the exhaustive pre-implementation net.** refine-spec's job is to catch **as many issues, missing requirements, and contradictions as possible before any code is written**: surface every internal contradiction and every conflict against the codebase, hunt the design details and edge cases the draft left implicit, and put every genuine gap or design fork to the **user** to decide — never silently guess. A requirement or contradiction caught here is cheap; one that survives into implementation is not. Cast the net **wide** (many parallel subagents, tailored lenses — below); the cost of an extra reviewer is far less than the cost of a missed requirement.
...
**Cast the adversarial net wide — scale it to the spec's risk surface.** Dispatch a **fresh** subagent (the `Task` tool — **never inline**; a session that has been editing the spec anchors on its own framing, which is exactly what fresh eyes avoid) as a **skeptic lens**: read the whole spec hunting for internal contradictions, **contradictions against the codebase**, over-engineering, speculative scope, anything an implementer could not act on, and **unstated non-functional constraints the change implies but never pins as an `AC`** — performance, security, idempotency, limits, concurrency (the requirements most often dropped). **For a `full` / infra / high-stakes spec, one generalist reviewer is not enough** — derive **3–5 tailored review lenses** for THIS spec's risk surfaces (blast-radius / irreversibility, data / PII correctness, dependency-ordering & bring-up, concurrency, API / contract, security, and always an implementability / spec-doc-quality lens) and dispatch **one fresh subagent per lens, in parallel**, each reading the whole spec + the codebase against its lens and returning only material findings. Several specialists cover far more ground than one generalist — more edge cases, more missing requirements, more contradictions surfaced — the breadth a wide fan-out of subagents buys. Fold every finding into Reconcile like any other.
...
For **every** genuine ambiguity, **internal contradiction or spec-vs-codebase conflict whose fix is a design choice** (not a mere wrong literal you can correct outright), unverifiable assumption, or open decision that changes *what gets built*, ask the user with `AskUserQuestion` — **surface it, never silently pick**. Batch related questions into one call; ask as many as you need across passes. **It is better to ask one too many questions than to let the spec ship an assumption.** Never guess to fill a gap. Walk dependent decisions in **dependency order** (resolve the upstream fork before the ones that hinge on it) and **lead each question with your recommended answer**. Grill while the answer still changes what gets built — but **stop when the returns diminish**: a cosmetic choice or an implementer's-discretion detail isn't worth a question.
...
Re-read the edited spec. Edits can introduce new claims, new ambiguities, or new contradictions. If the pass changed anything, **loop** and verify again. **Run the deterministic AC-integrity check every pass** — `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/spec_consistency.py <spec>` — and fix any **duplicate AC number or leftover conflict marker** it reports (exit 2, the blocking defects). It also prints **dangling `AC-N` reference** advisories to stderr (non-blocking): fix a real internal orphan, but a bare `AC-N` that points at a sibling spec is fine to leave. refine renumbers / splits / regroups ACs, so these creep in; the `Stop` hook runs the same check and **blocks finalization** on a blocking defect, so clear them as you go. If a full pass produced no fixes and no questions, evaluate the readiness gate with the independent judge (below) — don't sign off on your own work. Before you try to stop, make the ledger reflect reality — unresolved questions marked, gate flags set only where they truly hold. The `Stop` hook bounces you back here if anything is still open.
