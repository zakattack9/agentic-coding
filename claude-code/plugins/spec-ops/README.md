...
- **One format, scaled rigor — and a real front door.** `write-spec` is the workflow's **entrypoint**: at `full` rigor it first runs a short **discovery** pass that elicits and distills requirements from a bare idea (product questions sourced from the idea, *never* codebase grounding — that's `refine-spec`'s line), so it's the place you start with nothing but a thought, not just a transcriber. It then emits the same AC-first contract at three depths — `light` (a few atomic criteria, nothing else), `standard` (+ TL;DR and Boundaries), `full` (the exhaustive, self-contained deep spec that then goes to `refine-spec`). The criteria are always enumerated exhaustively; only the prose scales. **`light`/`standard` are also strictly code-free** — behavior and observable interfaces (routes, user-visible fields, API shapes) only, *no* internal implementation (file paths, symbols, table/columns, config keys) — so they stay implementation-agnostic and durable across refactors; `full` is where code-grounding (and the infra/config-as-contract exception) lives. A caller (e.g. a board-intake workflow) sets the rigor; direct use defaults to `full`.
