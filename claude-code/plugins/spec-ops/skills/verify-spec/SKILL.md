...
- **Availability — check this first.** Skill-load probe: !`python3 "${CLAUDE_PLUGIN_ROOT}/scripts/codex_bridge.py" --probe --kind judge-verify` — **if that line shows `CODEX: YES`, run the second judge.** Any other outcome — a `CODEX: NO …` line, a blank line, or an error / denied result — means Codex is unavailable: **skip this whole section** and proceed Claude-only (build no prompt, make no bridge call). Fail-open: a missing or denied probe never blocks verification.
