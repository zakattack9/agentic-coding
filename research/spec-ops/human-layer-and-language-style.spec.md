...
| AC  | Criterion                                                                                                                                                                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 30  | A Vietnamese output style file exists at this repo's `claude-code/output-styles/` directory, is tracked in git, and is valid, loadable Claude Code output-style syntax; it is NOT bundled into the spec-ops plugin (not in the plugin's `output-styles/`, not declared in `plugin.json`).                                |
...
| 33  | The style instructs that when explaining an English spec / checklist / file to the user, Claude translates the shown content into Vietnamese (keeping code tokens English) rather than quoting the English prose verbatim; it never rewrites the artifact on disk — only the spoken explanation is Vietnamese.           |
...
A single standalone style file at `claude-code/output-styles/` (repo-tracked), opt-in. Its instruction body must establish, unambiguously:
...
- **Translate what you show** — when walking the user through an English spec, checklist, or file, translate the shown content into Vietnamese (code tokens stay English) rather than quoting the English prose verbatim; the artifact on disk is never rewritten — only the spoken explanation is Vietnamese.
...
- [ ] Inspect `claude-code/output-styles/<style>.md` → tracked; valid loadable syntax; frontmatter has name + description + `keep-coding-instructions: true`; no force/auto-apply → (AC-30, AC-31)
...
- [ ] Ask Claude to explain the spec → it explains in Vietnamese, translating the shown content (code tokens stay English), and does not require you to read English → (AC-33, AC-35)
