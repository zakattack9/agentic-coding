...
| **Engineering quality + leave-it-better** | fixed | *"Implement to a real engineering-quality bar — architecture fit, clear code design, security, performance, error handling, tests, maintainability — **scaled to the change**. **Leave the touched surface no worse, and better where warranted:** if implementing cleanly would mean building on an existing **poor pattern**, prefer a **bounded** refactor of it — within these Boundaries and the spec's objective — over piling a new pattern on top. Keep it **proportionate**: don't gold-plate, don't redesign beyond the goal, and if a warranted refactor would cross a Boundary, **stop and surface it** rather than silently expand."* — new/updated code should leave the codebase in a **better state** so debt doesn't compound; this operates **inside** Boundaries, never past them |
...
**Copy the driver to the clipboard** so the handoff is a single ⌘V. After showing the prompt in chat, pipe the *exact same text* (**command prefix included** — the clipboard bytes are byte-identical to the chat bytes) to the clipboard following **`${CLAUDE_PLUGIN_ROOT}/references/clipboard-copy.md`** (the portable single-quoted heredoc, the session-gated tool selection, and the chat-only fallback all live there — single-sourced so `launch-spec` and `loop-spec` can't drift). Copying is not running — it stays emit-only. Driver-specific handling:
