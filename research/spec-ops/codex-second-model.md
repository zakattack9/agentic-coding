...
<!-- Grounded build order: every wiring needs the bridge (§1); the two judge wirings also follow the shared policy (§3). One coherent change with real cross-group dependencies — launch-spec phases the build by group. AC-ids are globally unique and stable across groups. -->

### 1. Codex bridge primitive — start here
...
| 3 | A review/judge invocation runs in a **strictly read-only sandbox** — Codex cannot modify any file, create a commit, or reach the network — enforced at the sandbox boundary (`--sandbox read-only`; `codex exec` is inherently non-interactive, so no approval flag is needed or accepted), not by prompt text. |
| 4 | The bridge **never** passes a sandbox/approval-escalating flag — `--dangerously-bypass-approvals-and-sandbox`, `--dangerously-bypass-hook-trust`, or a writable-scope `--add-dir` — nor moves `--sandbox` off `read-only`; their absence is guaranteed by construction. |
...
### 2. Enablement, graceful degradation & cost control — needs §1

| AC | Criterion |
|----|-----------|
| 8 | When Codex is installed **and** authenticated, the cross-model checks run **automatically**; when it is absent or unauthenticated, every skill behaves exactly as today (Claude-only), with **no error and no prompt**. Auth is probed **non-interactively** via `codex login status` (plus `OPENAI_API_KEY` / `CODEX_API_KEY`), never by attempting a login. |
| 9 | An explicit **off switch** (an environment variable, e.g. `SPEC_OPS_CODEX=0`) disables all Codex cross-model checks even when Codex is available; with it set, behavior is byte-behavior-identical to today. |
...
| 29 | The **write-discovery reviewer** (the only wiring on the interactive authoring path) has its **own independent toggle** (e.g. `SPEC_OPS_CODEX_WRITE=0`), so its single call can be disabled without disabling the verify/refine judges. |

### 3. Cross-model judge policy (shared by verify + refine) — needs §1
...
| 38 | Because the rubric file is passed **verbatim** (AC-36), the Codex judge inherits the skill's **exact materiality / diminishing-returns bar**, so a different-provider threshold does not manufacture low-value findings that churn the loop. |
| 39 | A Claude/Codex split that **doesn't resolve through the normal loop** (Claude's judge passes a criterion but Codex keeps failing it with no further fix available) **escalates to the user for disposition** — via `AskUserQuestion` when run interactively, or via the skill's normal blocked/handoff return when delegated under orchestrate-spec (which owns the question) — and the user's disposition **resolves the contested gate flag**, so the unchanged Stop hook (AC-40) can release. The loop never spins silently, and **a stubborn Codex FAIL can never deadlock the gate**. |
...
### 4. verify-spec wiring — needs §1, §3
...
### 5. refine-spec wiring — needs §1, §3
...
### 6. write-spec discovery requirements reviewer (advisory) — needs §1
...
**Invocation it builds** (review/judge role — read-only is the only mode used in this spec). `codex exec` is **inherently non-interactive** — it has no `--ask-for-approval` / `--full-auto` / `--yolo` flag in codex-cli 0.141.0 — so `--sandbox read-only` is the whole guarantee:
...
### Availability & auth guard (AC-8)

Before invoking, the bridge probes — **non-interactively, no API call, no browser**:

1. `command -v codex` — absent ⇒ exit `10`.
2. `codex login status` reports logged in, **or** `OPENAI_API_KEY` / `CODEX_API_KEY` is set — neither ⇒ exit `10`.

It never runs `codex login`. *(Every CLI flag in this spec was verified against the installed **codex-cli 0.141.0**.)*
