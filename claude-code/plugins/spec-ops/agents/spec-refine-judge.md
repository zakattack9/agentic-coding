...
- **`ac_complete`** — every functional requirement and constraint is captured in the AC table as a discrete, atomic, testable assertion; nothing load-bearing left only in prose; every criterion is **traced by ≥1 `## Checklist` item**. **At standard/full, the human layer is present and consistent** — a `## Summary` (code-free, plain-language, a derived view adding no fact beyond the table) and a two-subsection `## Checklist` (`### For agents` + `### For humans`) whose coverage across both subsections is exhaustive (every AC traced, no orphan trace). A missing or inconsistent human layer is a `Gap` here — **`FAIL`** it. **Exempt** a spec in the legacy flat code-area `## Checklist` shape (no subsections).
