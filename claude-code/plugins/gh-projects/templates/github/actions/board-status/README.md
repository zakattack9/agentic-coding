...
### AC-20 — native "PR merged" built-in target
...
`scaffold-repo` (a different phase) sets that built-in to `On Staging`; this
...
- uses: ./.github/actions/board-status
  with:
    ...
    app-private-key: ${{ secrets.PROJECTS_APP_PRIVATE_KEY }}
...
each issue, and **publishes the tag's Release** (AC-21).
