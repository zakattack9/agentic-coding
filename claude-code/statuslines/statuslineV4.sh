...
# ---- options ----
short_dir=1  # 1 = show only last N dirs, 0 = show full path
dir_levels=3 # number of trailing path components to show when short_dir=1
...
sessid_color() { if [ "$use_color" -eq 1 ]; then printf '\033[0;38;5;103m'; fi; }  # muted periwinkle-gray
ostyle_color() { if [ "$use_color" -eq 1 ]; then printf '\033[0;38;5;139m'; fi; }  # muted mauve-gray
...
# ---- shorten directory ----
# keep the last $dir_levels path components (grandparent/parent/current);
# shorter paths are shown as-is.
if [ "$short_dir" -eq 1 ] && [ "$current_dir" != "unknown" ]; then
  current_dir=$(echo "$current_dir" | awk -F/ -v levels="$dir_levels" '{
    n = NF
    start = (n > levels) ? n - levels + 1 : 1
    s = ""
    for (i = start; i <= n; i++) s = s (i > start ? "/" : "") $i
    print s
  }')
fi
...
# ---- render statusline ----
# line 1: directory, git, model, effort
...
# line 3: cc version, session id, output style
printf '\n '
row3_started=0
if [ -n "$cc_version" ] && [ "$cc_version" != "null" ]; then
  printf '%sv%s%s' "$(style_color)" "$cc_version" "$(rst)"
  row3_started=1
fi
if [ -n "$session_id" ] && [ "$session_id" != "null" ]; then
  [ "$row3_started" -eq 1 ] && printf ' %s✦ %s' "$(sep_color)" "$(rst)"
  printf '%s%s%s' "$(sessid_color)" "$session_id" "$(rst)"
  row3_started=1
fi
if [ -n "$output_style" ] && [ "$output_style" != "null" ]; then
  [ "$row3_started" -eq 1 ] && printf ' %s✦ %s' "$(sep_color)" "$(rst)"
  printf '%s%s%s' "$(ostyle_color)" "$output_style" "$(rst)"
  row3_started=1
fi
printf '\n'
