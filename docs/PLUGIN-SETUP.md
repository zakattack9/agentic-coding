...
This repo is a **single Git repository that acts as both the marketplace and the host for all plugins**. There is no separate marketplace repo. The `marketplace.json` at the root points to plugin subfolders via relative paths.
...
- **This repo doubles as its own marketplace.** Most docs describe the marketplace and plugins as separate repos. The pattern here — `marketplace.json` at root with `"source": "./plugins/<name>"` entries — collapses them into one repo. This is valid but not the primary example in the docs, so you'll need to synthesize from the marketplace source format docs and the plugin structure docs independently.
