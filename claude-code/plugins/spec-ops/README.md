...
| Skill | Does |
|-------|------|
| `/spec-ops:write-spec` | Draft a concise, scannable spec — **behavior, not implementation**; tables / mermaid / mockups over prose; say things once; opens with an id'd **Acceptance Criteria** table (the testable contract; optionally split into ordered named groups); an explicit **Boundaries** section (what NOT to touch). Scales to a requested **rigor** — `light` (AC-only) · `standard` (+ TL;DR + Boundaries) · `full` (the exhaustive deep spec). Asks before guessing via `AskUserQuestion` at `full`; lighter tiers defer unknowns to `[NEEDS CLARIFICATION]` markers. |
