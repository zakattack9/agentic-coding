...
When the target is a spec with an `## Acceptance Criteria` section, also run the *backward* direction: every **substantive** change in the implementation diff should map to an owning `AC-id`; a hunk that maps to **none** is the finding (scope creep, silent reinterpretation, or a derived requirement the forward judge can't see). Sweep the diff — an explicit PR / range, else `git merge-base HEAD main` → `git diff <base>..HEAD`, or `origin/main..HEAD` when you are on the trunk — and record the base in `backwardSweep.base` (or a `skippedReason`; **never block** when no base is determinable autonomously). **Substantive = behavior-bearing only**; refactors, formatting, tests, CI, config churn, and docs are allowlisted. For each unmapped hunk add a `findings` entry with `evidence`, a `proposedAC`, and a triage `disposition` (`intended` / `unintended` / `unsure`) — **always a non-blocking report; you edit nothing**. Full mechanics (manifest field-split, the AC-governed-doc exception, when it's safe to ask): `${CLAUDE_PLUGIN_ROOT}/references/verify-internals.md`.
...
- **Observe, never change — the tool grants are broader than the contract.** This skill holds `Bash` and `Write`, but its contract is strictly read-only:
  ...
  - **Write:** the **only** legal target is the `/tmp` ledger. Never write or edit a repo file, the spec, or code. (The drift baseline, the spec-amendment handoff, and a deterministic spec-linkage scan — all in `/tmp`, never the repo — are written by the `Stop` hook, never by you; you only *read* the drift baseline at run start.)
