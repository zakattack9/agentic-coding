...
Proceed only when the probe line above **shows `CODEX: YES`**. If it shows anything else — a
`CODEX: NO …` line, a blank line, or an error / denied result — Codex is unavailable: tell the
user (quote the line) and **stop** — do not compose a prompt, do not call the bridge, do not
review as Claude. A denied or failed probe is never a crash; it just means Codex couldn't be
reached.
