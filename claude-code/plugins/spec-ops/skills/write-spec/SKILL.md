...
**Availability — check this first.** Skill-load probe: !`python3 "${CLAUDE_PLUGIN_ROOT}/scripts/codex_bridge.py" --probe --kind write-requirements` — **if that line shows `CODEX: YES`, run the reviewer below.** Any other outcome — a `CODEX: NO …` line, a blank line, or an error / denied result — means Codex is unavailable: **skip the reviewer entirely** and proceed straight to committing the draft (build no prompt, make no bridge call). Fail-open: a missing or denied probe never blocks the draft.
