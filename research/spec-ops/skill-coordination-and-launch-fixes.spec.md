...
| 16  | The **dynamic-workflow** driver emits a **runnable `ultracode`-led prompt — never a JavaScript workflow script**. **`ultracode` is the leading token**, followed by a self-contained brief that **(a)** points the worker at the spec with a read directive, **(b)** states the measurable goal (every `AC-id` holds, grounded by `verify-spec`), and **(c)** describes the **workflow shape** — phased by AC groups, with `verify-spec` as the **final and per-phase gate** (and the sibling-judge dispatch where a workflow `agent()` can't spawn a `Task`). A single paste opts into orchestration and Claude **authors + runs** the workflow from the brief; launch-spec never emits a literal workflow script (Claude converts the prompt internally — emit-only is preserved). |
...
- [ ] Emitted workflow driver is a runnable `ultracode`-led prompt (read-directive · goal · workflow shape · `verify-spec` gate), **not** a JS workflow script — AC-16
