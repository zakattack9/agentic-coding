...
argument-hint: [what to verify: @spec.md | a PR/commit range | a claim] [focus areas] [--codex-effort xhigh|high|medium]
model: opus
effort: xhigh
...
**Flag a stale human layer.** For a spec target, the drift re-check also flags a **`## Summary`** or **`## Checklist`** gone stale relative to the **current ACs**: a checklist trace to an `AC-id` that no longer exists (removed / renamed), an AC no checklist item traces, or a Summary describing behavior the AC table no longer contains. Report it (read-only — `refine-spec` reconciles it); skip a spec still in the legacy flat code-area `## Checklist` shape.
...
```json
{
  ...
  "codeQualitySweep": {
    ...
    "findings": [
      {
        "file": "file:line of the delivered code",
...
- `method` records **how** the claim was grounded — one of the five techniques defined in the **Evidence standard** section below (`static-read` / `measurement` / `exhaustive-check` / `cli-observation` / `test-run`). Required for every `confirmed`/`contradicted` claim; it is what makes the evidence standard auditable (a measurable-threshold claim whose `method` is `static-read` is under-verified by construction — the judge flags it). Leave empty for `unverifiable`.
...
- `backwardSweep`, `specLinkageSweep`, and `codeQualitySweep` are the three **report-only sweeps**: like a `contradicted` claim they surface findings but **never block the stop**, and you edit nothing — the hook shape-validates the field but never gates on it; the judge attests the sweep ran. (This holds wherever any of the three appears below.)
...
- `codeQualitySweep` records the **code-quality / leave-it-better pass** (step 2): `ran` true once it has run, and `findings` the delivered code that ships materially worse than the change warranted — sloppy or debt-entrenching code, or a poor pattern the change built atop where a bounded refactor was warranted — each with its `file:line`, the `vertical` it fails (the `${CLAUDE_PLUGIN_ROOT}/references/quality-bar.md` verticals, or `leave-it-better`), the `concern`, and a `suggestion`. **Report-only** — it never gates (verify is read-only), it is the built-code twin of refine's debt-perpetuation check. Omit it when the target is not a spec implementation.
...
**Wait for every dispatch, then write the done-signal last.** Reach turn-end with a **fully reconciled** ledger: don't end the turn or write the done-signal — the per-claim results (`verdict` / `evidence` / `method` / `disposition`) plus `judge.ran` / `judge.verdict` / `judge.missed` / `judge.weakEvidence` — until every grounding `Explore` agent and (on the completeness pass) the Claude `spec-verify-judge` `Task` + the Codex bridge have **all returned** and each real result is folded in, as one reconciled write. Re-establish `judge.*` from **this** pass's results every pass (never carry a prior pass's `ran`/`verdict` forward unverified); a judge that errors or returns malformed output is re-dispatched or recorded, never a pass. The shape-only `backwardSweep` / `specLinkageSweep` / `codeQualitySweep` `ran` flags aren't part of the done-signal. Shared await / foreground / both-judges policy: **`${CLAUDE_PLUGIN_ROOT}/references/cross-model-judge.md`**.
...
**If the target is a spec with an `## Acceptance Criteria` section, seed the ledger with every `AC-id` as a claim first** — each criterion is the contract the implementation is judged against, so a missing `AC-id` is the worst kind of missed claim. Carry the `AC-id` in the claim text. Then add any other checkable claims from the body. The **`## Summary`** and **`### For humans`** checklist are **derived human views**, not sources of truth — do **not** enumerate their plain-language prose as independent claims; the `AC-id`s and body facts stay canonical. A criterion that grounds out `contradicted` is the exact "requirement built wrong / not built" finding this gate exists to surface. If the target genuinely has **no checkable claims**, say so plainly and stop (record one `unverifiable` claim noting that, dispositioned "no checkable claims in target") — don't invent claims to fill the ledger.

### 2. Verify — ground each claim against reality

Dispatch **parallel `Explore` subagents** (the `Task` tool, `subagent_type: Explore`) to ground the claims — they are read-only and fast. Split the claims by area and scale the agent count to the size of the work. Require each subagent to return **strict JSON — one object per claim** — and **validate the fields you depend on before writing them into the ledger** (never treat a subagent's prose as ground truth; pipe the return through `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/validate_return.py --kind grounder-verify` for a deterministic shape check — exit 2 prints the schema to re-dispatch against):

```json
[ { "claim": "…", "verdict": "confirmed | contradicted | unverifiable", "evidence": "file:line / git show <sha> / git diff hunk / read-only CLI output", "method": "static-read | measurement | exhaustive-check | cli-observation | test-run" } ]
```
...
Scale the evidence to **what the claim asserts** and record the technique in `method` — a bare code citation is not automatically enough: a **measurable threshold** → `measurement` (a code citation doesn't prove a bound); a **universal invariant** → `exhaustive-check` over the whole surface (not one compliant example); a plain **behavior** → `static-read` / `test-run`; an **infra / ops** fact → `cli-observation` (never forced into a test suite). This closes the gap where a perf/security constraint gets **rubber-stamped by code-reading**; the judge (step 4) flags any `method` below what the claim demands. Ground each claim's **behavior, not the mere existence of a symbol** — require the cited code to be reachable / wired, to hold under the cases the claim implies, and not to regress a sibling AC; an existence-grep comes back from the judge as `weakEvidence`. A **quality / leave-it-better AC** grounds like any other, and often demands the stronger standard: a *"pattern X removed / consolidated"* claim is a **universal invariant** → an **exhaustive-check** over the surface (the pattern is gone *everywhere*), not one refactored example (`${CLAUDE_PLUGIN_ROOT}/references/quality-bar.md`). **Read `${CLAUDE_PLUGIN_ROOT}/references/verify-internals.md`** for the full evidence-standard rules and both sweep mechanics — the worked detail behind this step.

#### Backward sweep — delivered code that owns no AC (report-only)

When the target is a spec with an `## Acceptance Criteria` section, also run the *backward* direction: every **substantive** change in the implementation diff should map to an owning `AC-id`; a hunk that maps to **none** is the finding (scope creep, silent reinterpretation, or a derived requirement the forward judge can't see). Resolve the diff base in order: an explicit PR / range / base sha handed in (wins), else — when the target is a committed spec — **the spec's defining commit `git log -1 --format=%H -- <spec>`** (the durable pre-implementation anchor; over-inclusion is safe and gets triaged out-of-scope), else `git merge-base HEAD main` → `git diff <base>..HEAD`, or `origin/main..HEAD` when you are on the trunk. Record the base in `backwardSweep.base` (or a `skippedReason`; **never block** when no base is determinable autonomously). **Substantive = behavior-bearing only**; refactors, formatting, tests, CI, config churn, and docs are allowlisted. For each unmapped hunk add a `findings` entry with `evidence`, a `proposedAC`, and a triage `disposition` (`intended` / `unintended` / `unsure`). Full mechanics (manifest field-split, the AC-governed-doc exception, when it's safe to ask): `${CLAUDE_PLUGIN_ROOT}/references/verify-internals.md`.

#### Spec-linkage hygiene sweep — artifact text that points back at the spec (report-only)

The spec is build scaffolding, not the product: flag delivered code / docs / comments / tests / output that still **reference the build spec** — `AC-id`s, build-phase / §-section numbers, spec ids or filenames, predecessor provenance ("salvaged from X"), or "newly / now / previously" build-increment framing. Run the deterministic detector over the diff (the base the backward sweep used) and write its hits into `specLinkageSweep.findings` (set `ran` true; a clean artifact is `ran` true, `findings: []`):

```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/spec_linkage_scan.py" --json --diff <base>
```

The detector is the greppable *first* pass — **also do the judgment pass**: read the diff for **`identifier`s** named after the spec / phase / predecessor rather than after what they do (`AC34_Foo`, `phase1_handler`), and inert **`background`** comments (what the code used to be, options rejected, build phasing), all while **keeping load-bearing rationale** (why this design, what breaks if it changes). Record these too (`patternType` `identifier` / `background`); the judge attests the judgment pass ran. Full keep-vs-strip guards + examples (and the standalone-linter mode): `${CLAUDE_PLUGIN_ROOT}/references/verify-internals.md`.

#### Code-quality sweep — did the delivered code ship worse than the change warranted? (report-only)

When the target is a spec implementation, also review the **implementation diff** (the base the backward sweep used) against the engineering-quality bar — the built-code twin of `refine-spec`'s debt-perpetuation check. For the surface the change touches, look for code that ships **materially worse than the change warranted**: a poor pattern the change **built atop** where a **bounded, warranted refactor** would have left the surface better (the `(a)(b)(c)` test in `${CLAUDE_PLUGIN_ROOT}/references/quality-bar.md`), or a materially-implied quality **vertical** (architecture-fit, code design, security, performance, scalability, maintainability, error-handling, observability, backward-compat) the code handled sloppily. Write each into `codeQualitySweep.findings` with its `file:line`, the `vertical`, the `concern`, and a `suggestion` (set `ran` true; clean code is `ran` true, `findings: []`). **This is report-only and never gates** — verify is read-only, and a quality AC the spec *did* require is graded as a forward claim like any other, not here. Keep it high-signal: flag code that entrenches real debt or fails a materially-implied vertical, not a style preference or a diminishing-returns nit (mirror the judge's materiality bar). Dispatch an `Explore` agent for the review when the diff is large. The verticals and the warranted-refactor `(a)(b)(c)` test are the sweep's mechanics: `${CLAUDE_PLUGIN_ROOT}/references/quality-bar.md`.
...
Also write the **backward sweep** into the ledger's `backwardSweep`: set `ran` true, record the `base` swept (or `skippedReason`), and list every unmapped substantive hunk in `findings`. Likewise write the **spec-linkage hygiene sweep** into `specLinkageSweep` and the **code-quality sweep** into `codeQualitySweep` (each `ran` true, findings listed) — all three are report-only sweeps.

### 4. Judge — an independent agent confirms completeness

**The agent that did the verifying does not get to declare it complete.** Before you try to stop, dispatch a fresh **verification judge** — the **`spec-ops:spec-verify-judge`** agent (the `Task` tool, `subagent_type: spec-ops:spec-verify-judge`) — with **no memory of your passes**; hand it **only the target and the ledger**, not your reasoning. Its adversarial rubric lives in the agent: read-only, it re-derives the checkable claims from the target itself, flags any missing from the ledger (`missed`) — confirming every `AC-id` is present as a claim when the target has an Acceptance Criteria section — and flags any `confirmed`/`contradicted` claim whose evidence is hollow, stale, doc-based, or **below the standard the claim demands** (`weakEvidence`). It also attests the **backward**, **spec-linkage**, and **code-quality** sweeps ran honestly — against the right base, with the judgment passes done (spec/history-named `identifier`s and inert `background` context; sloppy or debt-entrenching code), not just the greppable tokens — so a gap it spots there is its own pass, while those sweeps' *findings* never block. It returns strict JSON:

```json
{ "verdict": "complete | gaps", "missed": [...], "weakEvidence": [...], "backwardSweepAttested": true, "specLinkageSweepAttested": true, "codeQualitySweepAttested": true }
```

Validate the shape (`python3 ${CLAUDE_PLUGIN_ROOT}/scripts/validate_return.py --kind judge-verify`), then write `verdict` / `missed` / `weakEvidence` into the ledger's `judge` field and set `judge.ran` true. Every entry in `missed` / `weakEvidence` becomes another pass; the judge returns `complete` only when both are empty and all three sweeps ran honestly.

#### Cross-model judge — a second, different-provider judge (when available)

So a completeness check isn't *Claude auditing Claude*, run a second judge of a **different provider** (OpenAI Codex) **beside** the Claude judge on the completeness pass — **optional and fail-open**: when Codex is absent / unauthenticated / off / slow / malformed it is a no-op and the verdict is exactly what the Claude judge produced.

- **Availability — check this first.** Skill-load probe: !`python3 "${CLAUDE_PLUGIN_ROOT}/scripts/codex_bridge.py" --probe --kind judge-verify` — the probe prints one line; if it **shows `CODEX: YES`**, Codex is available — **run the second judge.** (Only the `CODEX: YES` verdict matters; whatever text follows it is an informational reason that may change — ignore it.) Any other line — a `CODEX: NO …` line, blank, or an error / denied result — means unavailable: **skip this whole section** and proceed Claude-only (build no prompt, make no bridge call) **and state in the handoff that the review ran Claude-only**. Fail-open: a missing or denied probe never blocks verification.
- **On `CODEX: YES`, follow `${CLAUDE_PLUGIN_ROOT}/references/cross-model-judge.md`** for the full mechanics — final-pass-only, **wait-for-every-dispatch-then-write-the-ledger-last**, **synchronous foreground** concurrent dispatch beside the Claude `spec-verify-judge` `Task` (never background-and-poll), the rubric-verbatim prompt, exit-code branching, AND-merge, stubborn-split escalation, and the **workflow-stage path when `Task`-spawn is unavailable**. **verify-spec bindings for that policy:** `--kind judge-verify`; `--schema-file ${CLAUDE_PLUGIN_ROOT}/schemas/judge_verify.schema.json`; rubric `${CLAUDE_PLUGIN_ROOT}/agents/spec-verify-judge.md`; inputs = the target + the current ledger only (no memory of prior passes); `--effort <the run's --codex-effort, else xhigh>` (the bridge also honors `SPEC_OPS_CODEX_EFFORT`). The AND-merge folds the **union** of both judges' findings into the single existing `judge` field — `verdict: "complete"` only when **both** judges return `complete`, `missed` / `weakEvidence` = the union — so `stop_verify_spec.py` and the ledger schema stay unchanged.
- **Reiterate Codex's findings to the user before folding them in:** for each `missed` / `weakEvidence` item Codex raised, say *"Codex flagged X; my own judge said Y; my disposition is Z."* A Codex-only finding must never be invisible — surface the split, don't union it silently.
- **Inside a workflow (no `Task`-spawn), never skip the cross-model check.** The Codex bridge is a plain `Bash` call — run it standalone (it works in a workflow agent); the independent Claude judge is then dispatched by the workflow as a **sibling `agent({ agentType: 'spec-ops:spec-verify-judge' })` stage**, not a nested `Task` (the build⇄verify workflow's JS owns that — see `orchestrate-spec` / `launch-spec`). If *neither* the bridge nor a judge stage could run, the verdict is **self-administered** (one model auditing itself) — say so loudly in the Handoff and recommend a fresh out-of-workflow verify-spec.
...
- [ ] If the target is a spec implementation, the **code-quality sweep** ran (`codeQualitySweep.ran`) and every place the delivered code ships materially worse than the change warranted is reported in `codeQualitySweep.findings`. Report-only, never a gate-blocker (judge-attested, not hook-enforced).
...
- **Coverage matrix** (when the target is a spec) — a compact matrix keyed on `AC-id`: `AC-id | verdict | method | evidence | checklist-item`. The **`checklist-item`** column maps each `AC-id` to the `## Checklist` item(s) that verify it **across both the `### For agents` and `### For humans` subsections** (read from each item's `(AC-…)` trace). An **empty cell is itself the finding**: an `AC-id` with no evidence is a forward gap, and one no checklist item traces is a criterion no check verifies. Also report the **checklist problems** you find while building the column — read-only, you flag and `refine-spec` fixes: a trace citing a non-existent `AC-id` (invalid / orphan), duplicate or conflicting coverage, and any checklist item that matches no AC. **A legacy flat `## Checklist`** (`code area → AC-id`, no `### For agents` / `### For humans` subsections) is itself a finding to report — `refine-spec` migrates it into the two-subsection form (`${CLAUDE_PLUGIN_ROOT}/references/spec-format.md`), so surface it rather than treating it as compliant. Keep the matrix *generated* from the ledger — never an author-maintained document.
...
- **Code quality / leave-it-better** (if that sweep ran) — each place the delivered code ships materially worse than the change warranted, with its `file:line`, the `vertical`, the concern, and a suggested improvement; or "the delivered code meets the bar" when it found nothing. Report-only — surfaced for the user or a follow-up refine pass, never auto-fixed.
...
- **Backward findings are a report-only sweep.** An unmapped substantive hunk is surfaced with a proposed AC and a triage — never auto-added to the spec, never a reason to reopen `refine-spec`. Keep the noise filter tight: behavior-bearing changes only; refactors, formatting, tests, CI, config churn, and docs are allowlisted.
