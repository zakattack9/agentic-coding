...
Lead with a **markdown table** of **acceptance criteria** — every behavior and constraint that must hold once the change is done, each row a single testable assertion. The `AC` column holds the **bare stable number** (`1`, `2`, …; cited as `AC-1`, `AC-2` everywhere else). This table is the spec's contract: the reader's two-minute scan of *what must be true*, and what the implementation is later gated against criterion-by-criterion. The full conventions — id format, grouping, the Checklist-as-index rule — are in `${CLAUDE_PLUGIN_ROOT}/references/ac-contract.md`; the rules that shape a **first draft**:
