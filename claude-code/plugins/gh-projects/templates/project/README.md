...
template** — fields, the 8 views, and the 9 Insights charts all came from that copy
...
- **Blocked** *(auto)*: `yes/no`, derived from native blocked-by edges by `lib/dag`.
...
- **9 Insights charts** — see `insights.md` (live numeric trends; UI-only, no API).

> Insights has **no API**: `scaffold-repo` can neither create nor verify charts —
> after scaffolding, **confirm the 9 charts are present** on this board by hand.
