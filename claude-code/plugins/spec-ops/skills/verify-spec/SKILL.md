...
- **Availability — check this first.** Skill-load probe: !`python3 "${CLAUDE_PLUGIN_ROOT}/scripts/codex_bridge.py" --probe --kind judge-verify` — if it reads `CODEX: NO`, **skip this whole section** (proceed Claude-only exactly as today: build no prompt, make no bridge call); only on `CODEX: YES` do the steps below.
